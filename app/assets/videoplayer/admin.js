...
function sliders_click_embedbox(){
	...
	var theid = ($t.prev().text());
        var ind = $t.parent().parent().parent().children().index($t.parent().parent());
        var sl = jQuery('.slider-con').eq(ind);
        var w = sl.find('.textinput[name='+ind+'-settings-width]').val();
        var h = sl.find('.textinput[name='+ind+'-settings-height]').val();
        
	$('#embedcode').html('<h4>iFrame Embed ( recommended )</h4><div class="code">&lt;iframe src="'+window.dzs_admin_loc+'bridge.php?action=view&id='+theid+'" width="'+w+'" height="'+h+'" style="overflow:hidden;" &gt;&lt;/iframe&gt;\
	</div>\
